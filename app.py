...
# ── Tab imports ─────────────────────────────────────────────────────────────
# Filenames do NOT match content. Aliases correct the mismatch:
# tab_pca.py → 資料總覽
# tab_feature_eng.py → 跨製程監控
# tab_feature_importance.py → 特徵⼯程 & 清理
# tab_overview.py → 缺失值分析
# tab_cross_process.py → 相關性分析
# tab_stat_test.py → PCA 分析
# tab_trend.py → 統計檢定
# tab_missing.py → ⽂獻佐證分析
# tab_correlation.py → empty placeholder (特徵重要性)
# tab_literature.py → duplicate 特徵⼯程, not used
from tabs import tab_pca as tab_overview
from tabs import tab_feature_eng as tab_cross_process
from tabs import tab_feature_importance as tab_feature_eng
from tabs import tab_overview as tab_missing
from tabs import tab_cross_process as tab_correlation
from tabs import tab_stat_test as tab_pca
from tabs import tab_trend as tab_stat_test
from tabs import tab_missing as tab_literature
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
...
"""
合併多個製程步驟：
• 單步驟 → 直接回傳 dfs_dict[step]（欄位已去掉前綴，原始⾏為不變）
...
default_multi = prev_steps if prev_steps else process_list[:min(2, len(process_li
...
if len(selected_steps) > 1:
st.info("多步驟模式：欄位保留 `步驟:參數` 完整名稱", icon=" ")
st.markdown("---")
...
with tabs[0]:
tab_cross_process.render(raw_df) # tab_feature_eng.py
with tabs[1]:
tab_overview.render(raw_df, dfs_dict, selected_process_df, selected_process) # tab_pca.p
with tabs[2]:
# tab_trend.py is 統計檢定, not 趨勢圖 — placeholder until correct file is available
st.info(" 趨勢圖分析功能即將上線。請確認 tab_trend.py 檔案內容是否正確。")
with tabs[3]:
tab_feature_eng.render(selected_process_df) # tab_feature_importance.py
with tabs[4]:
tab_missing.render(selected_process_df) # tab_overview.py
with tabs[5]:
tab_correlation.render(selected_process_df) # tab_cross_process.py
with tabs[6]:
tab_pca.render(selected_process_df) # tab_stat_test.py
with tabs[7]:
# tab_correlation.py is empty — placeholder for 特徵重要性
st.info(" 特徵重要性分析功能即將上線。")
with tabs[8]:
tab_stat_test.render(selected_process_df) # tab_trend.py
with tabs[9]:
tab_literature.render() # tab_missing.py
