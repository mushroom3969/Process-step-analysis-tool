...
def render(selected_process_df):
    ...
    with st.expander("🗂️ Train / Test Batch 分配", expanded=True):
        st.caption(
            "**Train**：用來建立 PCA 模型與控制限（請選已知正常批次）。　"
            "**Test**：投影到 Train 建立的空間，比對控制限以判斷是否異常。　"
            "若不選 Test，則全部批次都作為 Train（原始模式）。"
        )
        train_batches = st.multiselect(
            "✅ Train Batch（建模用，建議選正常批次）",
            options=all_batches,
            default=all_batches,          # 預設全選
            key="pca_train_batches",
        )
        remaining = [b for b in all_batches if b not in train_batches]
        test_batches = st.multiselect(
            "🔍 Test Batch（驗證用，投影到 Train 空間）",
            options=remaining,
            default=remaining,
            key="pca_test_batches",
        )
        ta, tb = st.columns(2)
        ta.info(f"Train：**{len(train_batches)}** 批")
        tb.info(f"Test：**{len(test_batches)}** 批")
