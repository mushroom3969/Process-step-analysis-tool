...
def _render_rf_tab(fi_subtab, X_fi, y_fi, top_n_fi):
    with fi_subtab:
        ...
        if st.button("🌲 訓練 Random Forest", key="run_rf"):
            with st.spinner("RF 訓練中..."):
                ...
                st.session_state.update({"fi_rf": rf, "fi_perm_df": perm_df, "fi_r2": r2,
                                         "shap_vals": None})
            # OOB R²
            oob_r2 = rf.oob_score_ if hasattr(rf, "oob_score_") else None
            # MDI importance
            mdi_df = pd.DataFrame({
                "Feature": X_fi.columns,
                "MDI_Importance": rf.feature_importances_,
            }).sort_values("MDI_Importance", ascending=False).reset_index(drop=True)
            st.session_state.update({"fi_mdi_df": mdi_df, "fi_oob_r2": oob_r2})
            oob_str = f"  OOB R² = {oob_r2:.3f}" if oob_r2 else ""
            st.success(f"✅ 完成！訓練 R² = {r2:.3f}{oob_str}")
        ...
        c2.metric("OOB R²（不偏估計）", f"{oob_r2:.3f}" if oob_r2 is not None else "—")
        c3.metric("特徵數", X_fi.shape[1])

        st.dataframe(perm_df.style.background_gradient(cmap="Blues", subset=["Perm_Importance"]),
                     width="stretch", hide_index=True)

        # MDI vs Permutation 比較
...
def _render_lasso_tab(fi_subtab, X_fi, y_fi, top_n_fi):
    with fi_subtab:
        ...
        with st.expander("📈 Regularization Path（Alpha vs 係數）", expanded=False):
            ...
            ax.axvline(np.log10(best_alpha), color="#e84855", ls="--", lw=1.5, label=f"最佳 α={best_alpha:.4f}")
    ...
    with fi_subtab:
        ...
        if st.button("📐 計算 PLS", key="run_pls_vip"):
            with st.spinner("PLS 訓練中..."):
                ...
                # Q²：cross-validated R²（不偏估計）
                from sklearn.model_selection import KFold
                kf = KFold(n_splits=min(5, len(y_arr)), shuffle=True, random_state=42)
                q2_scores = []
                for tr, te in kf.split(X_arr):
                    pls_cv = PLSRegression(n_components=n_pls)
                    pls_cv.fit(X_arr[tr], y_arr[tr])
                    q2_scores.append(r2_score(y_arr[te], pls_cv.predict(X_arr[te]).ravel()))
                q2_pls = float(np.mean(q2_scores))

                # Y explained variance per component
                y_var_per_comp = []
                for nc in range(1, n_pls + 1):
                    pls_tmp = PLSRegression(n_components=nc)
                    pls_tmp.fit(X_arr, y_arr)
                    y_var_per_comp.append(r2_score(y_arr, pls_tmp.predict(X_arr).ravel()))
                y_var_each = [y_var_per_comp[0]] + [y_var_per_comp[i] - y_var_per_comp[i-1]
            ...
            st.success(f"✅ 完成！R² = {r2_pls:.3f}　Q² = {q2_pls:.3f}")
        ...
        pm2.metric("Q²（CV，不偏）", f"{q2_pls:.3f}" if q2_pls is not None else "—",
        ...
        if y_var_each:
            ...
            ax_pls.set_ylabel("新增解釋 Y 變異 (%)")
            ax_pls.set_title("PLS 各主成分對 Y 的邊際解釋力")
...
def _render_mi_tab(X_fi, y_fi, top_n_fi):
    ...
    if st.session_state.get("mi_df") is None:
        return
    ...
    axes[1].barh([p - 0.18 for p in y_pos], merge["MI_norm"],     height=0.35,
                 color="#7209b7", alpha=0.85, label="MI (norm)")
    axes[1].barh([p + 0.18 for p in y_pos], merge["Pearson_norm"], height=0.35,
                 color="#2e86ab", alpha=0.85, label="|Pearson r| (norm)")
    axes[1].set_yticks(list(y_pos))
    axes[1].set_yticklabels(merge["Feature"].tolist(), fontsize=8)
    axes[1].set_xlabel("正規化分數（0-1）")
    axes[1].set_title("MI vs |Pearson r| 比較")
    ...
    st.markdown("#### 🔍 非線性信號偵測")
    st.caption(
        "**MI_norm - |Pearson|_norm > 0** 的特徵，代表線性相關低但資訊量高，"
        "可能存在非線性關係，值得在 SHAP Dependence Plot 進一步觀察。"
    )
    nonlinear_df = mi_df[mi_df["NonLinear_signal"] > 0.05].sort_values(
        "NonLinear_signal", ascending=False
    ).head(top_n_fi)
...
def _render_consensus_tab(X_fi, top_n_fi):
    st.markdown("#### 🏆 綜合重要性排名")
    st.caption(
        "整合所有已執行的方法（RF Permutation、Lasso、SHAP、PLS-VIP、Mutual Information），"
    ...
    if perm_df is not None and "Perm_Importance" in perm_df.columns:
        methods["RF_Perm"] = perm_df.set_index("Feature")["Perm_Importance"].reindex(X_fi.columns).fillna(0)

    lasso_df = st.session_state.get("lasso_coef_df")
    if lasso_df is not None and "Abs_Coef" in lasso_df.columns:
        methods["Lasso"] = lasso_df.set_index("Feature")["Abs_Coef"].reindex(X_fi.columns).fillna(0)

    shap_vals = st.session_state.get("shap_vals")
    if shap_vals is not None:
        mean_shap = np.abs(np.array(shap_vals)).mean(axis=0)
        methods["SHAP"] = pd.Series(mean_shap, index=X_fi.columns)

    vip_df = st.session_state.get("pls_vip_df")
    if vip_df is not None and "VIP" in vip_df.columns:
        methods["PLS_VIP"] = vip_df.set_index("Feature")["VIP"].reindex(X_fi.columns).fillna(0)

    mi_df = st.session_state.get("mi_df")
    if mi_df is not None and "MI" in mi_df.columns:
        methods["MI"] = mi_df.set_index("Feature")["MI"].reindex(X_fi.columns).fillna(0)
    ...
    consensus = score_df[["綜合分數"]].join(rank_df[["平均名次"]]).join(score_df[list(methods.keys())])
    ...
    if len(methods) >= 2:
        ...
        for i in range(n_m):
            for j in range(i+1, n_m):
                r, _ = spearmanr(rank_df[method_names[i]], rank_df[method_names[j]])
        ...
        ax3.set_yticklabels(method_names, fontsize=10)
        for i in range(n_m):
            for j in range(n_m):
...
def render(selected_process_df):
    ...
    fi_subtabs = st.tabs(["🌲 RF 重要性", "🔪 Lasso 重要性", "🔮 SHAP 分析", "📐 PLS-VIP", "🔄 交互作用探索", "🤝 Mutual Information", "🏆 綜合排名"])
